<?php
/**
 * PayHere return_url for treatment plan payments only.
 * Finalizes via process-treatment-plan-payment.php, then redirects to success UI.
 */
session_start();
...
$sessOrder = $_SESSION['tp_pay_order_id'] ?? '';
if ($orderId === '' || $sessOrder === '' || $orderId !== $sessOrder) {
    header('Location: patient_appointments.php?payment=session');
    exit;
}

$planId = (int) ($_SESSION['tp_pay_plan_id'] ?? 0);
$amount = (float) ($_SESSION['tp_pay_amount'] ?? 0);
$customerName = $_SESSION['tp_pay_name'] ?? '';
$customerEmail = $_SESSION['tp_pay_email'] ?? '';
$customerPhone = $_SESSION['tp_pay_phone'] ?? '';
...
$apiUrl = $protocol . '://' . $host . $basePath . '/public/api/process-treatment-plan-payment.php';
