...
$treatmentRescheduleToken = '[TREATMENT_RESCHEDULED_ONCE]';
    ...
    switch ($action) {
        ...
        case 'reschedule':
            ...
            if (in_array(($booking['status'] ?? ''), ['Confirmed', 'Completed'], true)) {
                echo json_encode(['success' => false, 'error' => 'Cannot reschedule after confirm/payment']);
                exit;
            }

            if (stripos((string)($booking['description'] ?? ''), $treatmentRescheduleToken) !== false) {
                echo json_encode(['success' => false, 'error' => 'Treatment can be rescheduled only once before payment']);
                exit;
            }
            
            $success = $model->rescheduleBooking($booking_id, $new_slot_id, $new_date);
            if ($success) {
                $markStmt = $conn->prepare("\n                    UPDATE treatment_bookings\n                    SET description = TRIM(CONCAT(COALESCE(description, ''), ' ', ?))\n                    WHERE booking_id = ?\n                ");
                if ($markStmt) {
                    $markStmt->bind_param('si', $treatmentRescheduleToken, $booking_id);
                    $markStmt->execute();
                    $markStmt->close();
                }
            }
